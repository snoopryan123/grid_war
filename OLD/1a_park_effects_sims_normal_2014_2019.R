...
##################################
### 2014-2019 Simulation Setup ###
##################################

### data matrix 
X_df = park_df %>% filter(YEAR >= 2014) %>% 
...
### everything relative to ANA park and ANA2014 
...
# m1 = lm(INN_RUNS ~ factor(OFF_TEAM_ID) + factor(DEF_TEAM_ID) + factor(PARK), data=park_df %>% filter(YEAR == 2014))
...
for (i in 1:NUM.SIMS) {
  ...
  ) %>% bind_rows(tibble(OT_YR = "ANA2014", beta_hat_OQ = 0)) %>% arrange(OT_YR)
  curr_dq_i = data.frame(
    DT_YR = str_sub(names(coeffs_dq_i), -7, -1),
    beta_hat_DQ = unname(coeffs_dq_i)
  ) %>% bind_rows(tibble(DT_YR = "ANA2014", beta_hat_DQ = 0)) %>% arrange(DT_YR)
...
ggsave("plot_13_1419.png", plot_13, width=8, height=6)
ggsave("plot_12_1419.png", plot_12, width=8, height=6)

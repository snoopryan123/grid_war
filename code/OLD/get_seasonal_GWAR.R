...
war2_og <- war2_ogg %>% filter(SP_IND | lag(SP_IND, default=FALSE))
...
expected_gwar_eoi <- function(i,j,exit_at_end) {
...
expected_gwar <- function(i,j,k,exit_in_mid) {
  ...
  for (aaa in 1:length(exit_in_mid)) {
    ...
    if (e == 0) {
      ...
      for(w in 0:max_inning_runs) {
        ss <- ss + f(i[aaa], k[aaa] + w) * g(j[aaa], w) #j needs to be the base state - Add one?
      }
    }
    result[aaa] = ss
  }
...
# # TESTS
# # "LAN201907190"
# expected_gwar(c(6), c("1 110"), c(0), c(1))
# expected_gwar(c(6), c("1 111"), c(1), c(1))
# # "CIN201908180"
# expected_gwar(c(6), c("0 100"), c(1), c(1))
